...
class PdoGsb{   		
      	private static $dsn = 'mysql:dbname=lionelfelvppedev;host=lionelfelvppedev.mysql.db';
      	private static $user = 'lionelfelvppedev';   		
      	private static $password = 'Welcom123'; 
 ...
 */				
	private function __construct(){
    	PdoGsb::$monPdo = new PDO(PdoGsb::$dsn, PdoGsb::$user, PdoGsb::$password); 
...
*/
    public function getInfosEmploye($login, $mdp){
        $req = "select id as idEmploye, nom, prenom, adresse, login, mdp ,dateEmbauche ,dateModifFicheE ,cptActif ,typeEmploye FROM employe where login='" . $login . "' and mdp='" . $mdp ."'";
...
*/
    public function getLesFraisHorsForfait($idVisiteur,$mois,$limit=NULL){
        $req = "select * from lignefraishorsforfait
                where emp_idEmploye ='$idVisiteur' and periodeConcernee = '$mois'";
        if ($limit != NULL){
...
*/
    public function getLesIdFrais(){
        $req = "select idFraisForfaitise as idfrais from fraisforfaitise where idFraisForfaitise not like 'KM%' order by idFraisForfaitise";
        $res = PdoGsb::$monPdo->query($req);
 ...
 */
    public function getIdFraisKm($libFraisKm){
        $req = "select idFraisForfaitise as idfrais from fraisforfaitise where idFraisForfaitise like 'KM%' and libelleFF like '%$libFraisKm%'";
        $res = PdoGsb::$monPdo->query($req);
        $lesLignes = $res->fetchAll(PDO::FETCH_ASSOC);
        return $lesLignes;
    }
    ...
    public function majFraisForfait($idVisiteur, $mois, $lesFrais){
        ...
        foreach($lesCles as $unIdFrais){
            ...
            $req = "update lignefraisforfaitise set lignefraisforfaitise.qteLigneFF = '$qte'
                    where lignefraisforfaitise.emp_idEmploye = '$idVisiteur' and lignefraisforfaitise.periodeConcernee = '$mois' and lignefraisforfaitise.idFraisForfaitise = '$unIdFrais'";
...
*/	
    public function estPremierFraisMois($idVisiteur,$mois){
        $ok = false;
        $req = "select count(*) as nbLignesFrais from fichefrais where periodeConcernee = '$mois' and emp_idEmploye = '$idVisiteur'";
        $res = PdoGsb::$monPdo->query($req);
...
*/	
    public function dernierMoisSaisi($idVisiteur){
        $req = "select max(periodeConcernee) as dernierMois from fichefrais where emp_idEmploye = '$idVisiteur'";
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetch();
...
/**
 * Crée une nouvelle fiche de frais et les lignes de frais forfaitisé pour un visiteur et un mois donné
...
*/
    public function creeNouveauFraisHorsForfait($idVisiteur,$mois,$libelle,$date,$montant){
        ...
        $req = "insert into lignefraishorsforfait(emp_idEmploye, periodeConcernee, libelleLigneFHF, montantLigneFHF, dateLigneFHF) values('$idVisiteur','$mois','$libelle','$montant','$dateFr')";
        PdoGsb::$monPdo->exec($req);
    }
/**
...
*/
    public function getLesMoisDisponibles($idVisiteur,$periodeActuelle){
        $req = "select fichefrais.periodeConcernee as mois from fichefrais where fichefrais.emp_idEmploye ='$idVisiteur' and fichefrais.periodeconcernee != '$periodeActuelle' order by fichefrais.periodeConcernee desc";
        $res = PdoGsb::$monPdo->query($req);
        $lesMois =array();
        $laLigne = $res->fetch();
        while($laLigne != null)	{
...
*/	
    public function getLesInfosFicheFrais($idVisiteur,$mois){
        $req = "select fichefrais.idEtatFiche as idEtat,fichefrais.dateModifFicheF as dateModif, fichefrais.nbrJustificatif as nbJustificatifs,
                ...
                left join employe on employe.id = fichefrais.idEmploye
                where fichefrais.emp_idEmploye ='$idVisiteur' and fichefrais.periodeConcernee = '$mois'";
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetch(PDO::FETCH_ASSOC);
        return $laLigne;
 ...
 */
    public function majEtatFicheFrais($idVisiteur,$mois,$etat,$montant_valide = 0,$idComptable = 0){
        $req = "update fichefrais set idEtatFiche ='$etat', dateModifFicheF = now() where fichefrais.emp_idEmploye ='$idVisiteur' and fichefrais.periodeConcernee ='$mois'";
        PdoGsb::$monPdo->exec($req);
        if ($etat == 'VA') {
 ...
 */
    public function getLesCptUtilisateur($idUtilisateur=NULL, $typeUtilisateur=NULL){
        ...
        if (!is_null($type)){
            $req .= " where employe.typeEmploye = '$type'";
        }
