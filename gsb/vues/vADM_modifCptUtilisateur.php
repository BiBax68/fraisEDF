        ...
        <form method="POST" action="index.php?uc=gererCptUtilisateur&action=validerMajCptUtilisateur">
            ...
            <label for="typeEmploye">Type employé :
                ...
                for ($i=0;$i<count($lesCatEmploye);$i++){
                    if ($lesCptUtilisateur[0]['idTypeEmploye'] == $lesCatEmploye[$i]['idTypeEmploye']){
                        $select = "selected";
                    }
                    else{
                        $select = "";
                    }
                    echo '<option value="' . $lesCatEmploye[$i]['idTypeEmploye'] . '"' . $select . '>' . $lesCatEmploye[$i]['libelleTypeEmploye'] . '</option>';
                }
                ?>
                </select>
            </label>
            ...
            if ($lesCptUtilisateur[0]['cptActif'] == 1){
                echo '  <label for="cptActif"><input type="radio" name="activationCptEmploye" id="cptActif" value="1" checked>Compte actif</label>
                        <label for="cptInactif"><input type="radio" name="activationCptEmploye" id="cptInactif" value="0">Compte inactif</label>';
            }
            else{
                echo '  <label for="cptActif"><input type="radio" name="activationCptEmploye" id="cptActif" value="1">Compte actif</label>
            ...
            <a href="index.php?uc=gererCptUtilisateur&action=voirCptUtilisateur"><button type="button" value="Annuler" name="btn_annuler">Annuler</button></a>
        </form>
    </div>
</div>